...
function AdminPanel({ user, onLogout }) {
  ...
  const fetchHistory = async (d) => {
    ...
    try {
      ...
      // Reverse array so the most recent trades show up at the top
      setHistTab({ data: res.data.reverse(), days: d, loading: false });
    } catch(e) {
      if (e.message.includes("warming up")) {
        flash("⏳ Engine is currently warming up indicators. Please wait a minute.");
      } else {
        flash("✗ History fetch failed.");
      }
      setHistTab(p => ({ ...p, loading: false }));
    }
  };

  // Auto-fetch history the first time the tab is opened
  useEffect(() => {
    if (tab === "history" && histTab.data.length === 0) {
      fetchHistory(histTab.days);
    }
  }, [tab]);
    ...
    <div style={{minHeight:"100vh",background:"#0b0e14",display:"flex",flexDirection:"column"}}>
      ...
      <div style={{padding:"20px 24px",display:"flex",flexDirection:"column",gap:16,maxWidth:1000,width:"100%",margin:"0 auto"}}>
        ...
        <div style={{borderBottom:"1px solid rgba(255,255,255,.07)",display:"flex",gap:4}}>
          ...
          <button style={tabStyle("history")}     onClick={()=>setTab("history")}>Historical Backtest</button>
        </div>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:16}}>
            ...
            <div style={{background:"rgba(255,255,255,.03)",border:"1px solid rgba(255,255,255,.09)",borderRadius:13,padding:"18px 22px",display:"flex",flexDirection:"column",gap:14}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                ...
                <div style={{display:"flex",gap:8}}>
                  {[1, 3, 5].map(d => (
                    <button key={d} onClick={() => fetchHistory(d)} 
                      style={{fontSize:10,padding:"4px 10px",borderRadius:4,background:histTab.days===d?"rgba(0,229,160,.2)":"rgba(255,255,255,.05)",color:histTab.days===d?"#00e5a0":"#c5ccd8",border:"none",cursor:"pointer",fontFamily:"'JetBrains Mono',monospace"}}>
                      {d} Day{d>1?'s':''}
                    </button>
                  ))}
                  <button onClick={() => fetchHistory(histTab.days)} 
                    style={{fontSize:10,padding:"4px 10px",borderRadius:4,background:"rgba(255,255,255,.05)",color:"#c5ccd8",border:"none",cursor:"pointer",fontFamily:"'JetBrains Mono',monospace"}}>
                    ↻ Refresh
                  </button>
                </div>
              </div>
            ...
            <div style={{background:"rgba(255,255,255,.02)",border:"1px solid rgba(255,255,255,.07)",borderRadius:13,overflow:"hidden"}}>
              <div style={{display:"grid",gridTemplateColumns:"140px 100px 100px 90px 90px",padding:"9px 14px",borderBottom:"1px solid rgba(255,255,255,.08)",background:"rgba(255,255,255,.03)"}}>
                {["Time","Price ₹","Sim Signal","Long %","Short %"].map(h=>(
                  <span key={h} style={{fontSize:10,color:"#5a6478",letterSpacing:".08em",textTransform:"uppercase"}}>{h}</span>
                ))}
              </div>
              <div style={{maxHeight:350,overflowY:"auto"}}>
                {histTab.loading ? (
                   <div style={{padding:"32px 0",textAlign:"center",color:"#5a6478",fontSize:12}}>Loading history from Engine...</div>
                ) : histTab.data.length === 0 ? (
                   <div style={{padding:"32px 0",textAlign:"center",color:"#5a6478",fontSize:12}}>No history available for this period.</div>
                ) : histTab.data.map((s,i) => {
                   
                   // Recalculate the signal on the fly based on the local sliders!
                   let sig = "WAIT";
